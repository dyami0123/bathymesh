...
from bathy.core.mesh_generators import (
    BaseMeshGenerator,
    FlatMeshGenerator,
    ExtrudedMeshGenerator,
    HeightmapMeshGenerator,
)
from bathy.triangulation import TriangleTriangulator
from bathy.core import HeightmapProcessor, MeshCombiner, HeightmapHandler
from bathy.data_structures import MeshScaling
...
class TestFlatMeshGenerator:
    ...
    def setup_method(self):
        ...
        self.generator = FlatMeshGenerator(
            self.triangulator, self.processor, self.combiner
        )
    ...
    def test_generate_mesh_simple_bounding_box(self, simple_heightmap, default_scaling):
        ...
        mesh = self.generator.generate_mesh(
            heightmap_handler, height=1.0, use_contours=False
        )
    ...
    def test_generate_mesh_with_contours(self, test_heightmap, default_scaling):
        ...
        heightmap_handler = HeightmapHandler(
            heightmap=test_heightmap, scaling=default_scaling, processor=self.processor
        )

        mesh = self.generator.generate_mesh(
            heightmap_handler, height=0.5, use_contours=True, threshold=0.3
        )
    ...
    def test_generate_mesh_contours_missing_threshold(
        ...
        with pytest.raises(ValueError, match="threshold"):
            self.generator.generate_mesh(
                heightmap_handler, height=0.5, use_contours=True, threshold=None
            )
    ...
    def test_generate_mesh_different_heights(self, simple_heightmap, default_scaling):
        ...
        for height in heights:
            mesh = self.generator.generate_mesh(
                heightmap_handler, height=height, use_contours=False
            )
...
class TestExtrudedMeshGenerator:
    ...
    def test_generate_mesh_simple(self, simple_heightmap, default_scaling):
        ...
        mesh = self.generator.generate_mesh(
            heightmap_handler, base_height=0.0, thickness=2.0, use_contours=False
        )
...
class TestHeightmapMeshGenerator:
    ...
    def test_generate_mesh_with_scaling(self, simple_heightmap, custom_scaling):
        ...
        heightmap_handler = HeightmapHandler(
            heightmap=simple_heightmap, scaling=custom_scaling, processor=self.processor
        )
    ...
    def test_generate_mesh_flat_heightmap(self, flat_heightmap, default_scaling):
        ...
        heightmap_handler = HeightmapHandler(
            heightmap=flat_heightmap, scaling=default_scaling, processor=self.processor
        )
    ...
    def test_generate_mesh_minimum_size(self):
        ...
        heightmap_handler = HeightmapHandler(
            heightmap=minimal_heightmap, scaling=MeshScaling(), processor=self.processor
        )
...
class TestMeshGeneratorIntegration:
    ...
    def test_all_generators_produce_valid_meshes(self, test_heightmap, default_scaling):
        ...
        heightmap_handler = HeightmapHandler(
            heightmap=test_heightmap, scaling=default_scaling, processor=processor
        )
    ...
    def test_mesh_properties_consistency(self, simple_heightmap, default_scaling):
        ...
        heightmap_handler = HeightmapHandler(
            heightmap=simple_heightmap, scaling=default_scaling, processor=processor
        )
