...
from bathy.workflows.multi_level_mesh_generation_workflow import MultiLevelMeshConfig
from bathy.workflows.simple_mesh_generation_workflow import SimpleMeshConfig
from bathy.data_structures import MeshType, MeshScaling
...
class TestHeightmapGenerationConfig:
    ...
    def test_custom_config(self):
        ...
        config = HeightmapGenerationConfig(
            size=100, central_peak=2.0, noise_level=0.5, add_zero_border=False
        )
...
class TestDebugVisualizationConfig:
    ...
    def test_custom_config(self):
        """Test custom configuration."""
        config = DebugVisualizationConfig(
            enabled=False, colormap="viridis", open_in_slicer=True
        )
...
class TestDebugWorkflow:
    ...
    def test_heightmap_generation_no_border(self, temp_dir):
        ...
        config = DebugWorkflowConfig(
            heightmap_config=HeightmapGenerationConfig(size=50, add_zero_border=False),
            mesh_config=SimpleMeshConfig(mesh_type=MeshType.SURFACE),
    ...
    def test_heightmap_generation_with_noise(self, temp_dir):
        ...
        config = DebugWorkflowConfig(
            heightmap_config=HeightmapGenerationConfig(
                size=50, noise_level=0.5, add_zero_border=False
            ),
            mesh_config=SimpleMeshConfig(mesh_type=MeshType.SURFACE),
            visualization_config=DebugVisualizationConfig(enabled=False),
            verbose=False,
    ...
    def test_visualization_with_contours(self, temp_dir):
        ...
        config = DebugWorkflowConfig(
            heightmap_config=HeightmapGenerationConfig(size=30),
            mesh_config=MultiLevelMeshConfig(
                thresholds=[0.0, 0.5, 1.0], mesh_type=MeshType.FLAT
            ),
            visualization_config=DebugVisualizationConfig(
                enabled=True, output_directory=viz_dir, show_contour_lines=True
            ),
            verbose=False,
        )
    ...
    def test_quick_run(self, temp_dir):
        ...
        result = DebugWorkflow.quick_run(
            mesh_type=MeshType.SURFACE, size=30, output_path=temp_dir / "quick_mesh.stl"
        )
    ...
    def test_result_str_representation(self, temp_dir):
        ...
        result = DebugWorkflow.quick_run(mesh_type=MeshType.SURFACE, size=30)
...
class TestDebugWorkflowIntegration:
    ...
    def test_end_to_end_surface_mesh(self, temp_dir):
        ...
        config = DebugWorkflowConfig(
            heightmap_config=HeightmapGenerationConfig(
                size=50, central_peak=1.0, noise_level=0.0
            ),
            mesh_config=SimpleMeshConfig(
                mesh_type=MeshType.SURFACE, save_output=True, output_path=output_file
            ),
            visualization_config=DebugVisualizationConfig(
                enabled=True, output_directory=viz_dir, clear_before_run=True
            ),
            verbose=False,
        )
