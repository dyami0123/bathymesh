...
from bathy.triangulation import BaseTriangulator, TriangleTriangulator, GmshTriangulator
...
class TestTriangleTriangulator:
    ...
    def test_multiple_polygon_error(self):
        ...
        with pytest.raises(
            ValueError, match="Multiple polygon triangulation not yet implemented"
        ):
    ...
    def test_large_polygon(self):
        ...
        assert (
            len(triangles) >= n_sides - 2
        )  # Polygon triangulation creates n-2 triangles minimum
...
class TestGmshTriangulator:
    ...
    def test_polygon_with_multiple_holes(self):
        ...
        assert (
            len(vertices) >= 12
        )  # At least outer (4) + hole1 (4) + hole2 (4) vertices
