...
from bathymesh.io import export_mesh, get_mesh_info
from bathymesh.data_structures import MeshInfo, MeshFormat
...
class TestGetMeshInfo:
    ...
    def test_cube_mesh_info(self):
        ...
        np.testing.assert_array_almost_equal(info.bounding_box_min, [0.0, 0.0, 0.0], decimal=5)
        np.testing.assert_array_almost_equal(info.bounding_box_max, [1.0, 1.0, 1.0], decimal=5)
        np.testing.assert_array_almost_equal(info.bounding_box_extent, [1.0, 1.0, 1.0], decimal=5)
    ...
    def test_mesh_info_fields(self):
        ...
        assert (
            isinstance(info.bounding_box_min, list) or info.bounding_box_min is None
        )
        assert (
            isinstance(info.bounding_box_max, list) or info.bounding_box_max is None
        )
        assert (
            isinstance(info.bounding_box_extent, list) or info.bounding_box_extent is None
        )
